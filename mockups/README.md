# Propuestas de diseño — página de inicio

Seis maquetas HTML autocontenidas de la página de inicio del sitio de la **Dra. Dalila Peñaranda** (Semana 2 del cronograma de la propuesta). Abrir `index.html` para navegarlas, o cada archivo directamente en el navegador — funcionan sin conexión (fuentes auto-hospedadas en `assets/fonts/`).

Versión 2 (tras la primera revisión con el cliente): la propuesta 1 se conserva; las otras cuatro fueron reemplazadas por mundos ilustrados e inmersivos definidos con `/impeccable shape`. Las cuatro originales quedaron en `archivo/`.

| # | Archivo | Dirección visual |
|---|---------|------------------|
| 1 | `1-mundo-de-juego.html` | **Mundo de juego** — alusiva a la pediatría, como un cuento infantil: cielo, sol, colinas y los colores del logo en tono lúdico. |
| 2 | `2-acuarela-de-cuento.html` | **Acuarela de cuento** — libro álbum pintado a mano: pigmento SVG con bordes irregulares, grano de papel, manzano en acuarela, notas Caveat. |
| 3 | `3-teatro-de-papel.html` | **Teatro de papel** — diorama pop-up: escenarios de cartulina por capas con profundidad, telón, sol colgado de un hilo, actos de una función. |
| 4 | `4-mapa-de-aventura.html` | **Mapa de aventura** — globo aerostático sobre el mapa del crecimiento: valle, campamentos, bosque, lago y puerto unidos por ruta punteada. |
| 5 | `5-jardin-del-amanecer.html` | **Jardín del amanecer** (premium) — grabado botánico en oro suave sobre porcelana: rosa antiguo, salvia y pétalos a la deriva; claro y elegante. |
| 6 | `6-un-dia-completo.html` | **Un día completo** — un solo paisaje del amanecer a la noche: cada momento del día es un tema de la doctora y la cita se agenda antes de apagar la luz. |

El brief de la dirección 6 está en `briefs/6-un-dia-completo.md` (ya construida). El jardín de medianoche (versión oscura del premium) quedó en `archivo/` junto a las cuatro propuestas de la primera ronda.

## Versión 5 — cuatro escenografías del teatro elegido

La doctora eligió el **teatro de papel**. Tras la observación del cliente de que las cuatro opciones anteriores eran el mismo diseño con distintos colores, las opciones B, C y D se reconstruyeron como **escenografías estructuralmente distintas** que conservan la esencia del teatro (cartulina por capas, telón, la **pediatra como protagonista** —su retrato o silueta marca el lugar de la fotografía real— y las fotos reales del consultorio en `assets/3d/`). Las versiones recoloreadas quedaron en `archivo/` (`*-teatro-recolor-v2.html`).

| # | Archivo | Escenografía |
|---|---------|--------------|
| A | `3a-teatro-consultorio.html` | **Claro cálido** — la puesta clásica por actos del teatro elegido, en paleta arena, azul-lila y menta. |
| B | `3b-teatro-arrecife.html` | **Teatro de marionetas** — la doctora-marioneta presenta la función; credenciales en cuerda, fotos con pinzas, cursos en carrito ambulante. |
| C | `3c-teatro-oceano.html` | **Libro pop-up** — el cuento desplegable: la doctora emerge del pliegue, solapas para levantar, álbum con esquineros, cintas marcapáginas y boletos recortables. |
| D | `3d-consultorio-real.html` | **Diorama continuo** — sin tarjetas: un mar de papel que se profundiza al hacer scroll, de la superficie al cofre abisal. |
| E | `3e-mar-de-papel.html` | **Mar de papel** — el teatro mezclado con el diseño de referencia del cliente. Los **stickers marinos son los reales del consultorio** (ballena, tortuga, caballito de mar, estrellas, corales y pez, recortados de las fotos y el video en `assets/3e/stickers/`); **Pepe**, el sapo de peluche que acompaña al consultorio desde su apertura, es la mascota oficial (recorte real + foto real de la doctora abrazándolo). Estructura de la referencia: servicios, conoce a la doctora, dúo nutrición/pediatría, consultorio, cursos, «DalyTips»/recursos, testimonios y citas. Las fotos de la doctora en consulta siguen siendo provisionales (generadas) hasta recibir las reales. |

## Notas

- **Todo el contenido es placeholder**: textos, credenciales, cursos, precios y testimonios son de ejemplo y están marcados como tales. Se reemplazan con el material real que entregue la doctora (Semana 1 del cronograma).
- Todas comparten las secciones del mapa del sitio de la propuesta: presentación y credenciales, cursos destacados (Hotmart), últimos artículos del blog, testimonios, citas (presencial/virtual con precios COP/USD) y contacto, más botón flotante de WhatsApp y aviso médico.
- El logo en `assets/` proviene del PDF oficial de entrega final (`Logo A finales.pdf`, diseño de Luz Angarita); cada PNG lleva su procedencia embebida (chunk `impeccable:prompt`).
- Son maquetas de diseño, no el sitio final: el sitio se construirá en WordPress sobre la dirección elegida, con dos rondas de ajustes incluidas.
